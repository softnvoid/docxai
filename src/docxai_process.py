...
def write_changes_paragraph(doc, paragrhaph_dict_modified: dict):
    for i in range(len(doc.paragraphs)):
        if not paragrhaph_dict_modified.get(str(i)) or (
            paragrhaph_dict_modified[str(i)] == doc.paragraphs[i].text
        ):
            continue
        else:
            ...
            for word in find_changes_generator(
                tmp_original_text, paragrhaph_dict_modified[str(i)]
            ):
                if word.startswith("###"):
...
def send_prompt(
    ...
    for _ in range(4):
        ...
        try:
            ...
            tmp_json = json.loads(
                tmp_json_response["result"]["alternatives"][0]["message"]["text"].strip(
                    "[]`json\n "
                )
            )
    ...
    if not success:
        logging.error(
            f"this chunck wiil be missed due to incorrect JSON formatting or missing elements: {chunk}"
        )
...
def func_yandexgpt(input_dict: dict) -> dict:
    ...
    for chunk in json_strings:
        ...
        if response:
            ...
            logging.info(
                f"json text {response_json['result']['alternatives'][0]['message']['text']}"
            )
            raw_str_json += (
                response_json["result"]["alternatives"][0]["message"]["text"].strip(
                    "[]}{`json\n "
                )
            ) + ","
    ...
    for main_i in input_dict.keys():
        ...
        for chunk in json_strings:
            ...
            if response:
                ...
                raw_str_json += (
                    (
                        response_json["result"]["alternatives"][0]["message"][
                            "text"
                        ].strip("[]}{`json\n ")
                    )
                    + "}"
